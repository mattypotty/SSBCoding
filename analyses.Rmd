...
```{r MI_mcmcglmm, eval=TRUE, echo=TRUE, results=TRUE}
...
```{r  NAC_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
...
```{r  NACf_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
...
```{r  NACm_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
...
```{r  SPI_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  SSB <- setNames(data$SSB,data$Species)
...
```{r  SPIf_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  FSSB <- setNames(data$FSSB,data$Species)
  SPI <- setNames(data$SPI,data$Species)
...
```{r  SPIm_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  MSSB <- setNames(data$MSSB,data$Species)
  SPI <- setNames(data$SPI,data$Species)
...
```{r  TSP_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  SSB <- setNames(data$SSB,data$Species)
...
```{r  TSPf_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  FSSB <- setNames(data$FSSB,data$Species)
  TSP <- setNames(data$TSP,data$Species)
...
```{r  TSPm_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  MSSB <- setNames(data$MSSB,data$Species)
  TSP <- setNames(data$TSP,data$Species)
...
```{r MGS_mcmcglmm, eval=TRUE, echo=TRUE, results=TRUE}
 if (RERUN) {
  #Read in data
...
### Hypothesis 2: 
Social species with highly coordinated and structured mating systems will be more intentional and selective in their mate choice, decreasing SSB prevalence.
...
```{r SDT_mcmcglmm, eval=TRUE, echo=TRUE, results=TRUE}
 if (RERUN) {
  #Read in data
...
```{r  SDT_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  SSB <- setNames(data$SSB,data$Species)
...
```{r  SDTf_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  FSSB <- setNames(data$FSSB,data$Species)
  SDT <- setNames(data$SDT,data$Species)
...
```{r  SDTm_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  MSSB <- setNames(data$MSSB,data$Species)
  SDT <- setNames(data$SDT,data$Species)
...
### Hypothesis 4: 
Solitary species that demonstrate occasional grouping are more likely to have same-sex interactions than purely solitary species, and are thus more likely to exhibit SSB.
...
```{r SOG_mcmcglmm, eval=TRUE, echo=TRUE, results=TRUE}
 if (RERUN) {
  #Read in data
...
```{r  SOG_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  SSB <- setNames(data$SSB,data$Species)
...
```{r  SOGf_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  FSSB <- setNames(data$FSSB,data$Species)
  SOG <- setNames(data$SOG,data$Species)
...
```{r  SOGm_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  MSSB <- setNames(data$MSSB,data$Species)
  SOG <- setNames(data$SOG,data$Species)
...
```{r RDM_mcmcglmm, eval=TRUE, echo=TRUE, results=TRUE}
 if (RERUN) {
  #Read in data
...
```{r  RDM_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  SSB <- setNames(data$SSB,data$Species)
...
```{r  RDMf_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  FSSB <- setNames(data$FSSB,data$Species)
  RDM <- setNames(data$RDM,data$Species)
...
```{r  RDMm_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  MSSB <- setNames(data$MSSB,data$Species)
  RDM <- setNames(data$RDM,data$Species)
...
```{r AM_mcmcglmm, eval=TRUE, echo=TRUE, results=TRUE}
 if (RERUN) {
  #Read in data
...
```{r  FAM_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  SSB <- setNames(data$SSB,data$Species)
  FAM <- setNames(data$FAM,data$Species)
  
   # Fit models where SSB and FAM are totally independent or dependent
  fit_SSB_FAM <- phytools::fitPagel(phy,SSB,FAM)
  fit_SSB_FAM
  plot(fit_SSB_FAM,lwd.by.rate=TRUE)
  
  # Fit model where SSB depends on FAM
  fit_SSB <- phytools::fitPagel(phy,SSB,FAM,dep.var="x")
  fit_SSB
  plot(fit_SSB,lwd.by.rate=TRUE)
  
  # Fit model where FAM depends on SSB
  fit_FAM <- phytools::fitPagel(phy,SSB,FAM,dep.var="y")
  fit_FAM
  plot(fit_FAM,lwd.by.rate=TRUE)
  
  # Comparing the goodness of all 4 models using AIC
  aic <- setNames(c(fit_SSB_FAM$independent.AIC,
                    fit_SSB$dependent.AIC,
                    fit_FAM$dependent.AIC,
                    fit_SSB_FAM$dependent.AIC),
                  c("independent","dependent SSB","dependent FAM","dependent SSB & FAM"))
  aic
  aic.w(aic)
}
```

```{r  FAMf_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  FSSB <- setNames(data$FSSB,data$Species)
  FAM <- setNames(data$FAM,data$Species)
  
   # Fit models where FSSB and FAM are totally independent or dependent
  fit_FSSB_FAM <- phytools::fitPagel(phy,FSSB,FAM)
  fit_FSSB_FAM
  plot(fit_FSSB_FAM,lwd.by.rate=TRUE)
  
  # Fit model where FSSB depends on FAM
  fit_FSSB <- phytools::fitPagel(phy,FSSB,FAM,dep.var="x")
  fit_FSSB
  plot(fit_FSSB,lwd.by.rate=TRUE)
  
  # Fit model where FAM depends on SSB
  fit_FAM <- phytools::fitPagel(phy,FSSB,FAM,dep.var="y")
  fit_FAM
  plot(fit_FAM,lwd.by.rate=TRUE)
  
  # Comparing the goodness of all 4 models using AIC
  aic <- setNames(c(fit_FSSB_FAM$independent.AIC,
                    fit_FSSB$dependent.AIC,
                    fit_FAM$dependent.AIC,
                    fit_FSSB_FAM$dependent.AIC),
                  c("independent","dependent FSSB","dependent FAM","dependent FSSB & FAM"))
  aic
  aic.w(aic)
}
```

```{r  FAMm_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  MSSB <- setNames(data$MSSB,data$Species)
  FAM <- setNames(data$FAM,data$Species)
  
   # Fit models where MSSB and FAM are totally independent or dependent
  fit_MSSB_FAM <- phytools::fitPagel(phy,MSSB,FAM)
  fit_MSSB_FAM
  plot(fit_MSSB_FAM,lwd.by.rate=TRUE)
  
  # Fit model where MSSB depends on FAM
  fit_MSSB <- phytools::fitPagel(phy,MSSB,FAM,dep.var="x")
  fit_MSSB
  plot(fit_MSSB,lwd.by.rate=TRUE)
  
  # Fit model where FAM depends on MSSB
  fit_FAM <- phytools::fitPagel(phy,MSSB,FAM,dep.var="y")
  fit_FAM
  plot(fit_FAM,lwd.by.rate=TRUE)
  
  # Comparing the goodness of all 4 models using AIC
  aic <- setNames(c(fit_MSSB_FAM$independent.AIC,
                    fit_MSSB$dependent.AIC,
                    fit_FAM$dependent.AIC,
                    fit_MSSB_FAM$dependent.AIC),
                  c("independent","dependent MSSB","dependent FAM","dependent MSSB & FAM"))
  aic
  aic.w(aic)
}
```

```{r  MAM_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  SSB <- setNames(data$SSB,data$Species)
  MAM <- setNames(data$MAM,data$Species)
  
   # Fit models where SSB and MAM are totally independent or dependent
  fit_SSB_MAM <- phytools::fitPagel(phy,SSB,MAM)
  fit_SSB_MAM
  plot(fit_SSB_MAM,lwd.by.rate=TRUE)
  
  # Fit model where SSB depends on MAM
  fit_SSB <- phytools::fitPagel(phy,SSB,MAM,dep.var="x")
  fit_SSB
  plot(fit_SSB,lwd.by.rate=TRUE)
  
  # Fit model where MAM depends on SSB
  fit_MAM <- phytools::fitPagel(phy,SSB,MAM,dep.var="y")
  fit_MAM
  plot(fit_MAM,lwd.by.rate=TRUE)
  
  # Comparing the goodness of all 4 models using AIC
  aic <- setNames(c(fit_SSB_MAM$independent.AIC,
                    fit_SSB$dependent.AIC,
                    fit_MAM$dependent.AIC,
                    fit_SSB_MAM$dependent.AIC),
                  c("independent","dependent SSB","dependent MAM","dependent SSB & MAM"))
  aic
  aic.w(aic)
}
```

```{r  MAMf_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  FSSB <- setNames(data$FSSB,data$Species)
  MAM <- setNames(data$MAM,data$Species)
  
   # Fit models where FSSB and MAM are totally independent or dependent
  fit_FSSB_MAM <- phytools::fitPagel(phy,FSSB,MAM)
  fit_FSSB_MAM
  plot(fit_FSSB_MAM,lwd.by.rate=TRUE)
  
  # Fit model where FSSB depends on MAM
  fit_FSSB <- phytools::fitPagel(phy,FSSB,MAM,dep.var="x")
  fit_FSSB
  plot(fit_FSSB,lwd.by.rate=TRUE)
  
  # Fit model where MAM depends on SSB
  fit_MAM <- phytools::fitPagel(phy,FSSB,MAM,dep.var="y")
  fit_MAM
  plot(fit_MAM,lwd.by.rate=TRUE)
  
  # Comparing the goodness of all 4 models using AIC
  aic <- setNames(c(fit_FSSB_MAM$independent.AIC,
                    fit_FSSB$dependent.AIC,
                    fit_MAM$dependent.AIC,
                    fit_FSSB_MAM$dependent.AIC),
                  c("independent","dependent FSSB","dependent MAM","dependent FSSB & MAM"))
  aic
  aic.w(aic)
}
```

```{r  MAMm_pagel, eval=TRUE, echo=TRUE, results=TRUE}
if (RERUN) {
  #Assign species names
  MSSB <- setNames(data$MSSB,data$Species)
  MAM <- setNames(data$MAM,data$Species)
  
   # Fit models where MSSB and MAM are totally independent or dependent
  fit_MSSB_MAM <- phytools::fitPagel(phy,MSSB,MAM)
  fit_MSSB_MAM
  plot(fit_MSSB_MAM,lwd.by.rate=TRUE)
  
  # Fit model where MSSB depends on MAM
  fit_MSSB <- phytools::fitPagel(phy,MSSB,MAM,dep.var="x")
  fit_MSSB
  plot(fit_MSSB,lwd.by.rate=TRUE)
  
  # Fit model where MAM depends on MSSB
  fit_MAM <- phytools::fitPagel(phy,MSSB,MAM,dep.var="y")
  fit_MAM
  plot(fit_MAM,lwd.by.rate=TRUE)
  
  # Comparing the goodness of all 4 models using AIC
  aic <- setNames(c(fit_MSSB_MAM$independent.AIC,
                    fit_MSSB$dependent.AIC,
                    fit_MAM$dependent.AIC,
                    fit_MSSB_MAM$dependent.AIC),
                  c("independent","dependent MSSB","dependent MAM","dependent MSSB & MAM"))
  aic
  aic.w(aic)
}
```
...
```{r IUCN_mcmcglmm, eval=TRUE, echo=TRUE, results=TRUE}
 if (RERUN) {
  #Read in data
