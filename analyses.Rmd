...
```{r dimorphicmorphology_mcmcglmm, eval=TRUE, echo=TRUE, results=TRUE}
# This code chunk runs an MCMCglmm analysis
...
if (RERUN) {
  ...
  # Runs MCMCglmm fancy linear regression analysis
  MCMCanalysis <- MCMCglmm::MCMCglmm(SSB~RDM+1, # Formula for analysis: SSB is related to RDM and some intercept
                  random = ~Species, # Random effect -- species identity is a confounding factor in the analysis
                  family = "categorical", # Our response variable is categorical
                  ginverse = list(Species=inv.phylo), # These are our phylogenetic relationships, which cause covariance structure
                  prior = prior, # Telling the MCMC what our priors are
...
```{r  RDMpagel, eval=TRUE, echo=TRUE, results=TRUE}
...
if (RERUN) {
  SSB <- setNames(data$SSB,data$Species) # Naming our SSB values with species names
  RDM <- setNames(data$RDM,data$Species) # Naming our RDM values with species names
  
  fit_SSB_RDM <- phytools::fitPagel(phy,SSB,RDM) # Fitting models where SSB and RDM are totally independent or dependent
  fit_SSB_RDM
  plot(fit_SSB_RDM,lwd.by.rate=TRUE)
  
  fit_SSB <- phytools::fitPagel(phy,SSB,RDM,dep.var="x") # Fitting models where SSB depends on RDM
  fit_SSB
  plot(fit_SSB,lwd.by.rate=TRUE)
  
  fit_RDM <- phytools::fitPagel(phy,SSB,RDM,dep.var="y") # Fitting models where RDM depends on SSB
  fit_RDM
