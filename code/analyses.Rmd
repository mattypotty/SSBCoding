...
library(birdring) # For analysing posterior distributions
```
...
process_mcmc <- function(ssbtype,hypothesis) {
  ...
  results <- data.frame(matrix(NA,nrow=0,ncol=0))
  for (file in files) {
  ...
  for (param in params) {
    ...
    val <- round(mean(results[[param]]),2)
    mean_text <- paste0("Mean: ",val)
    
    min <- abs(min(results[[param]]))
    max <- abs(max(results[[param]]))
    ...
    plot <- ggplot(results) +
      ...
      annotate("label",label=mean_text,x=xpos,y=1,hjust=hjust,vjust=1,fill="white",size=3) +
...
```{r process_hyp, echo=TRUE, eval=TRUE, purl=FALSE}
process_hyp <- function(hypothesis) {
  files <- Sys.glob(paste0("../output/MCMCGLMM_","*_",hypothesis,"/*.log.csv"))
  results <- data.frame(matrix(NA,nrow=0,ncol=0))
  for (file in files) {
    name_str <- strsplit(strsplit(basename(file),"\\.")[[1]][1],"_")[[1]]
    if (name_str[4] != "TREEMCC") {
      number <- name_str[5]
      ssbtype <- name_str[2]
      log <- read.csv(file)
      log$number <- number
  ...
  colnames(results)[1] <- "Intercept"
  results$ssbtype <- factor(results$ssbtype,levels=c("SSB","FSSB","MSSB"))
  params <- colnames(results)[1:(ncol(results)-2)]
  ...
  for (param in params) {
    color_list <- c()
    val_list <- c()
    for (ssbtype in c("SSB","FSSB","MSSB")) {
      results_subset <- results[which(results$ssbtype==ssbtype),]
      hpd95 <- hdi(results_subset[[param]],credMass=.95)
      hpd90 <- hdi(results_subset[[param]],credMass=.9)
      hpd80 <- hdi(results_subset[[param]],credMass=.8)
      hpd_vals <- sort(c(hpd80[[1]],hpd90[[1]],hpd95[[1]],hpd95[[2]],hpd90[[2]],hpd80[[2]],0))
      if (hpd_vals[1] == 0) {sig=4}
      if (hpd_vals[2] == 0) {sig=3}
      if (hpd_vals[3] == 0) {sig=2}
      if (hpd_vals[4] == 0) {sig=1}
      if (hpd_vals[5] == 0) {sig=2}
      if (hpd_vals[6] == 0) {sig=3}
      if (hpd_vals[7] == 0) {sig=4}
      colors <- c("black","gold","darkorange","red")
      color_list <- c(color_list,colors[sig])
      val_list <- c(val_list,round(mean(results_subset[[param]]),2))
    }
    
    mean_text <- paste0("SSB: ",val_list[1],"\n",
                        "FSSB: ",val_list[2],"\n",
                        "MSSB: ",val_list[3])
    ...
    plot <- ggplot(results) +
      geom_vline(xintercept=0) +
      geom_density(aes_string(x=param,y="..density..",group="ssbtype",lty="ssbtype",color="ssbtype")) +
      scale_color_manual(values=color_list,guide=FALSE) +
      labs(lty="SSB Type") +
      annotate("label",label=mean_text,x=xpos,y=Inf,hjust=hjust,vjust=1.1,fill="white",size=2.5) +
      ggtitle(paste0(param)) +
      theme_bw() +
      theme(plot.title=element_text(hjust=0.5),
            axis.title=element_blank(),
 	          #legend.position="none",
  ...
  big_plot <- wrap_plots(plot_list,nrow=1,guides="collect")
  ggsave(paste0("../figures/",hypothesis,".png"),big_plot,dpi=600,width=3*length(plot_list),height=3,limitsize=FALSE)
}
```
...
```{r MI_process, echo=TRUE, purl=FALSE}
process_hyp("MI")
```

```{r MI_show, eval=TRUE, results="SHOW", purl=FALSE}
include_graphics("../figures/MI.png")
```
...
```{r NAC_process, echo=TRUE, purl=FALSE}
process_hyp("NAC")
```

```{r NAC_show, eval=TRUE, results="SHOW", purl=FALSE}
include_graphics("../figures/NAC.png")
```
...
```{r MGS_process, echo=TRUE, purl=FALSE}
process_hyp("MGS")
```

```{r MGS_show, eval=TRUE, results="SHOW", purl=FALSE}
include_graphics("../figures/MGS.png")
```
...
```{r HAR_process, echo=TRUE, purl=FALSE}
process_hyp("HAR")
```

```{r HAR_show, eval=TRUE, results="SHOW", purl=FALSE}
include_graphics("../figures/HAR.png")
```
...
```{r SDT_process, echo=TRUE, purl=FALSE}
process_hyp("SDT")
```

```{r SDT_show, eval=TRUE, results="SHOW", purl=FALSE}
include_graphics("../figures/SDT.png")
```
...
```{r SOC_process, echo=TRUE, purl=FALSE}
process_hyp("SOC")
```

```{r SOC_show, eval=TRUE, results="SHOW", purl=FALSE}
include_graphics("../figures/SOC.png")
```
...
```{r RDM_process, echo=TRUE, purl=FALSE}
process_hyp("RDM")
```

```{r RDM_show, eval=TRUE, results="SHOW", purl=FALSE}
include_graphics("../figures/RDM.png")
```
...
```{r AM_process, echo=TRUE, purl=FALSE}
process_hyp("AM")
```

```{r AM_show, eval=TRUE, results="SHOW", purl=FALSE}
include_graphics("../figures/AM.png")
```
...
```{r IUCN_process, echo=TRUE, purl=FALSE}
process_hyp("IUCN")
```

```{r IUCN_show, eval=TRUE, results="SHOW", purl=FALSE}
include_graphics("../figures/IUCN.png")
```
...
for (file in files) {
  ...
  results <- log[c("speciation.1.","speciation.2.","speciation.3.","speciation.4.","extinction.1.","extinction.2.","extinction.3.","extinction.4.","speciation_observed.1.","speciation_observed.2.","extinction_observed.1.","extinction_observed.2.")]
  ...
  splim <- quantile(c(results$speciation_observed.1.,results$speciation_observed.2.),0.99)
  exlim <- quantile(c(results$extinction_observed.1.,results$extinction_observed.2.),0.99)
  
  spoverlap <- sprintf("%.2f",round(overlap(results$speciation_observed.1.,results$speciation_observed.2.,0,1),2))
  exoverlap <- sprintf("%.2f",round(overlap(results$extinction_observed.1.,results$extinction_observed.2.,0,1),2))
  
  plot_both_sp <- ggplot(results) +
    geom_density(aes(x=speciation_observed.1.,y=..scaled..),bounds=c(0,1),color="red") +
    geom_density(aes(x=speciation_observed.2.,y=..scaled..),bounds=c(0,1),color="blue") +
    xlim(c(0,splim)) +
    annotate("label",label=spoverlap,x=splim,y=1,hjust="inward",vjust="inward",fill="white",size=4) +
    ggtitle(clade) +
    theme_bw() +
    theme(plot.title=element_text(hjust=0.5),
          axis.title=element_blank(),
          ...
          legend.position="none",
          aspect.ratio=1/3)
  plot_both_ex <- ggplot(results) +
    geom_density(aes(x=extinction_observed.1.,y=..scaled..),bounds=c(0,1),color="red",lty="dotted") +
    geom_density(aes(x=extinction_observed.2.,y=..scaled..),bounds=c(0,1),color="blue",lty="dotted") +
    xlim(c(0,exlim)) +
    annotate("label",label=exoverlap,x=exlim,y=1,hjust="inward",vjust="inward",fill="white",size=4) +
    theme_bw() +
    theme(axis.title=element_blank(),
          ...
          legend.position="none",
          aspect.ratio=1/3)
  both_plot_list[[ind]] <- plot_both_sp / plot_both_ex
...
big_plot_both <- wrap_plots(both_plot_list,ncol=4)
ggsave(paste0("../figures/SSE.png"),big_plot_both,dpi=600,width=16,height=14)
```

```{r SSE_show, eval=TRUE, results="SHOW", purl=FALSE}
include_graphics("../figures/SSE.png")
```
