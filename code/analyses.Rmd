...
```{r process_mcmc, echo=TRUE, eval=TRUE}
process_mcmc <- function(ssbtype,hypothesis) {
  files <- Sys.glob(paste0("../output/MCMCGLMM_",ssbtype,"_",hypothesis,"_*.log.csv"))
...
```{r process_pagel, echo=TRUE, eval=TRUE}
process_pagel <- function(ssbtype,variable) {
  files <- Sys.glob(paste0("../output/PAGEL_",ssbtype,"_",variable,"_*.sum.txt"))
  names <- c("Independent", paste0("Dependent ",ssbtype), paste0("Dependent ",variable), "Interdependent")

  file_contents <- readLines(paste0("../output/PAGEL_",ssbtype,"_",variable,"_TREEMCC_001.sum.txt"))
...
```{r finished, results='hold', purl=FALSE}
mcmcglmm <- data.frame(matrix(NA,nrow=0,ncol=4))
...
colnames(mcmcglmm) <- c("SSB","HYP","TREE","NUM")

pagel <- unique(pagel)
colnames(pagel) <- c("SSB","VAR","TREE","NUM")

nums <- c("001", "002", "003", "004", "005", "006", "007", "008", "009", "010")
...
for (ssb in unique(mcmcglmm$SSB)) {
  for (hyp in unique(mcmcglmm$HYP)) {
    ...
    missing_1 <- nums[!(nums %in% numbers_1)]
...
for (file in files) {
  ...
  results <- log[c("sp0","sp1","ex0","ex1")]
  
  plot_sp <- ggplot(results) +
    geom_density(aes(x=sp0,y=..scaled..),color="blue") +
    geom_density(aes(x=sp1,y=..scaled..),color="red") +
    ggtitle(clade) +
    theme_bw() +
    theme(plot.title=element_text(hjust=0.5),
          axis.title=element_blank(),
  ...
  plot_ex <- ggplot(results) +
    geom_density(aes(x=ex0,y=..scaled..),color="blue") +
    geom_density(aes(x=ex1,y=..scaled..),color="red") +
    ggtitle(clade) +
    theme_bw() +
    theme(plot.title=element_text(hjust=0.5),
          axis.title=element_blank(),
